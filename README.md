...
- 当前数据保存在安装该 PWA 的设备中。支持把记录、照片和背景导出为 JSON 备份，再导入恢复。

## 1.0 功能

- 关注艺人的添加、重命名和删除。
- 现场记录的新建、编辑、删除、状态管理与自动次数统计。
- 行政区足迹地图、城市点亮、缩放和当地记录查看。
- 自定义背景持久保存，现场照片及票根在本机压缩保存。
- 订单截图通过 Tesseract.js 在浏览器端识别，识别后由用户核对字段。
